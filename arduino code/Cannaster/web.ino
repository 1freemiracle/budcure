void handleRoot() {
    ...
    "<p>Current humidity: <strong>" + String(humidity) + " °C</strong></p>"
    "<form action='/setVariables' method='POST'>"
    "<label for='maxHumidity'>max humidity:</label>"
    "<input type='number' id='maxHumidity' name='maxHumidity' min='1' value='" + String(maxHumidity) + "'>"
    "<label for='pressureThreshold'>pressure min:</label>"
    "<input type='number' id='pressureThreshold' name='pressureThreshold' min='1' value='" + String(pressureThreshold) + "'>"
    "<label for='prgDelay'>keep pump off for:</label>"
    "<input type='number' id='prgDelay' name='prgDelay' min='1' value='" + String(prgDelay) + "'>"
...
void handleSetVariables() {
    if (server.hasArg("maxHumidity")) {
        maxHumidity = server.arg("maxHumidity").toInt();
        EEPROM.put(MAX_ADDRESS, maxHumidity);
        EEPROM.commit();
    }
    if (server.hasArg("pressureThreshold")) {
        pressureThreshold = server.arg("pressureThreshold").toInt();
        EEPROM.put(PRESS_ADDRESS, pressureThreshold);
        EEPROM.commit();
    }
    if (server.hasArg("prgDelay")) {
      prgDelay = server.arg("prgDelay").toInt();
